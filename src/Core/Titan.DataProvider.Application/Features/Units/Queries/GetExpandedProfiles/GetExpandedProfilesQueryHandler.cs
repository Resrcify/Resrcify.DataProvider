...
public sealed class GetExpandedProfilesQueryHandler : IRequestHandler<GetExpandedProfilesQuery, IEnumerable<GetExpandedProfilesQueryResponse>>
{
    private readonly ICachingService _caching;
    public GetExpandedProfilesQueryHandler(ICachingService caching)
        => _caching = caching;

    public async Task<IEnumerable<GetExpandedProfilesQueryResponse>> Handle(GetExpandedProfilesQuery request, CancellationToken cancellationToken)
    {
        var baseData = await _caching.GetAsync<BaseData>($"BaseData-{request.Language}", cancellationToken);
        if (baseData is null) return Enumerable.Empty<GetExpandedProfilesQueryResponse>();
    ...
    private static IEnumerable<GetExpandedProfilesQueryResponse> GetExpandedPlayerProfiles(GetExpandedProfilesQuery request, BaseData baseData)
    {
        foreach (var profile in request.PlayerProfiles)
        {
            ...
            if (!request.WithoutDatacrons) datacrons = ExpandedDatacron.Create(profile.Datacrons, baseData);
