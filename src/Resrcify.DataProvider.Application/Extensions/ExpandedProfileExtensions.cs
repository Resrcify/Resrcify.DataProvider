using Resrcify.DataProvider.Domain.Internal.ExpandedUnit.Enums;
...
public static class ExpandedProfileExtensions
{
    public static DatacronSummary ParseDatacronSummary(this IEnumerable<ExpandedDatacron> expandedDatacrons)
    {
        var level0 = 0;
        var level1 = 0;
        var level2 = 0;
        ...
        foreach (var datacron in expandedDatacrons)
        {
            ...
            if (datacron.ActivatedTiers == 9)
                level9++;
            if (datacron.IsFocused)
                focused++;
            if (datacron.IsFocused && datacron.ActivatedTiers == datacron.MaxTiers)
                focusedMaxed++;
            rerolls += datacron.RerollCount;
        }
        return new DatacronSummary(
            level0,
            level1,
            level2,
            level3,
    ...
    public static ProfileSummary ParseSummaryData(
        this IEnumerable<KeyValuePair<string, ExpandedUnit>> expandedUnits,
        DatacronSummary datacronSummary)
    {
        ...
        foreach (var unit in expandedUnits)
        {
            ...
            ExtractModData(
                ref sixDotModsCount,
                ref speedUnder10,
                ref speedBetween10And14,
                ref speedBetween15And19,
                ref speedBetween20And24,
                ref speed25Plus,
                ref offenceBetween4And6Percent,
                ref offenceOver6Percent,
                unit);
        ...
        var gear = new GearSummary(
            gear1 - ships,  // Ships are all GearTier1
            gear2,
            gear3,
            gear4,
            gear5,
    ...
    private static void ExtractModData(
        ref int sixDotModsCount,
        ref int speedUnder10,
        ref int speedBetween10And14,
        ref int speedBetween15And19,
        ref int speedBetween20And24,
        ref int speed25Plus,
        ref int offenceBetween4And6Percent,
        ref int offenceOver6Percent,
        KeyValuePair<string, ExpandedUnit> unit)
    {
    ...
    private static void ExtractSkillData(
        ref int zetaCount,
        ref int omicronCount,
        ref int tbOmicronCount,
        ref int twOmicronCount,
        ref int gaOmicronCount,
        ref int cqOmicronCount,
        ref int raidOmicronCount,
        KeyValuePair<string, ExpandedUnit> unit)
    {
